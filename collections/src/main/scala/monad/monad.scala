...
package object monad {
  ...
  /**
   * Реализуйте методы pure / map / flatMap / withFilter чтобы работал код и законы монад соблюдались
  ...
  trait Wrap[+A] {
    ...
    def flatMap[R](f: A => Wrap[R]): Wrap[R] = this match {
      case EmptyWrap => EmptyWrap // Если Wrap пустой, то flatMap должен вернуть пустой Wrap
    ...
    def withFilter(f: A => Boolean): Wrap[A] = this match {
      case EmptyWrap => EmptyWrap // Если Wrap пустой, то withFilter должен вернуть пустой Wrap
